# Version 2.0.4 release: ? | last edit: 10/28/24
# Built by cdUAH
# Cleaned 3/20/25
    ...
    def custom_range():
        ...
        while True:
            try:
                ...
                # below is the same code used in to_epoch(), but adjusted for the input format from AMS notifications
                dt_s = datetime.strptime(time_range_input_start, '%Y-%j.%H:%M:%S')  # change depending on AMS format
                dt_s = dt_s.timestamp()
                dt_e = datetime.strptime(time_range_input_end, '%Y-%j.%H:%M:%S')  # change depending on AMS format
                dt_e = dt_e.timestamp()
                return [dt_s, dt_e]
